...
spec =
  describe "JWE encoding and decoding" $ do
    context "when using JWE Appendix 1 data" $ do
      let a1Header = defHdr {jwtAlg = RSA_OAEP, jwtEnc = Just A256GCM}
    ...
    context "when using JWE Appendix 2 data" $ do
      let a2Header = defHdr {jwtAlg = RSA1_5, jwtEnc = Just A128CBC_HS256}
...
jweRoundTrip g (JWEAlgs a e) msg = encodeDecode == Right (defHdr {jwtAlg = a, jwtEnc = Just e}, msg)
  where
...
data JWEAlgs = JWEAlgs Alg Enc deriving Show
